...
namespace ComsiteDesk.ERP.PublicInterface.Controllers
{
    ...
    {
        public ITicketTypesService _ticketTypesService { get; set; }
        ...
        {
            var items = _ticketTypesService.GetAllWithPager(searchParameters);

            return Ok(new { data = items, count = searchParameters.totalCount });
        }

        // GET: api/TicketTypes/All
        [HttpGet]
        [Route("All")]
        public async Task<ActionResult> Get()
        {
            var items = await _ticketTypesService.GetAllAsync();
        ...
        {
            var item = await _ticketTypesService.GetById(id);
            if (item == null)
            {
                return NotFound();
            }
        ...
        [HttpPost]
        public async Task<ActionResult> Post([FromBody] TicketTypesModel value)
        {
            ...
            var id = await _ticketTypesService.Add(value);
        ...
        [HttpPut("{id}")]
        public async Task<ActionResult> Put([FromBody] TicketTypesModel value)
        {
            var currentValue = await _ticketTypesService.GetById(value.Id);
            ...
            var item = _ticketTypesService.Update(value);
        ...
        {
            var existingItem = await _ticketTypesService.GetById(id);
            ...
            _ticketTypesService.Remove(existingItem);
