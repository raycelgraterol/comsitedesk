...
export class ListStatusComponent implements OnInit {
  ...
  tables$: Observable<ProjectStatusModel[]>;
  total$: Observable<number>;
  item: ProjectStatusModel;
  @ViewChildren(AdvancedSortableDirective) headers: QueryList<AdvancedSortableDirective>;

  constructor(
    public service: ProjectStatusService,
  ...
  ngOnInit() {
    ...
    this.service.getAll();
    this.item = new ProjectStatusModel();

    // tslint:disable-next-line: max-line-length
    this.breadCrumbItems = [
      { label: 'Comsite', path: '/' },
      { label: 'Estatus', path: '/', active: true }
    ];
